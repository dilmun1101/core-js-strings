...
function removeFirstOccurrences(str, value) {
  ...
  const endIndexValue = startIndexValue + value.length;
  if (startIndexValue === -1) {
    return str;
  }
  return str.slice(0, startIndexValue) + str.slice(endIndexValue, str.length);
}
...
function orderAlphabetically(str) {
  ...
  return arrOfCharacters.sort().join('');
}
